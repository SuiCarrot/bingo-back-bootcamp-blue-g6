...
export default class Card{
    ...
    constructor(){
        ...
        this.numbers = baseNumbers.sort(() => Math.random() - 0.5).slice(0,25)
    }
    
    getNumbers(){
        const cardNumbers = this.numbers

        return cardNumbers
    }

    getCombination(positions: number[], cardNumbers: number[]): number[]{
        
        return positions.map(position => cardNumbers[position])
    }
    
    checkCombination(wholeSet: number[], subSet: number[]): boolean {
        for(let item of subSet) if(!wholeSet.includes(item)) return false
        return true
    }

    checkVictory(drawnNumbers: number[], cardNumbers: number[]): boolean{
        for(let positions of Constants.cardPositions){
            let combination = this.getCombination(positions, cardNumbers)
